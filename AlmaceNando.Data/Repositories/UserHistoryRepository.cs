...
namespace AlmaceNando.Data.Repositories
{
    public class UserHistoryRepository:IUserHistoryReposiory
    {
        ...
        public async Task<UserHistory?> GetLastOpening(DateTime dateTime, int Action)
        {
            // Buscamos la última apertura global para cerrar el bloque de tiempo actual
            return await _context.Set<UserHistory>()
                .OrderByDescending(element => element.CreatedAt)
                .FirstOrDefaultAsync(T => T.CreatedAt < dateTime && T.actionType == Action);
        }
    }
}
